...
class UTKDataset(Dataset):
    ...
    def __init__(self, dataFrame, transform=None):
        ...
        self.age_bin = np.array(dataFrame.agebin[:])
    ...
    def __getitem__(self, index):
        ...
        labels = torch.tensor((self.age_bin[index], self.eth_label[index]))
...
def get_args():
    ...
    parser.add_argument('--dataset', default='UTK')
    parser.add_argument('--method', default='MGDA')
    parser.add_argument('--n_tasks', type=int,default=2)
    parser.add_argument('--gradnorm', default='loss+', type=str, choices=['l2', 'loss','loss+','none'])
...
args = get_args()
if args.dataset=='UTK':
    mu=torch.tensor((0,0,0)).float().view(3,1,1).cuda()
    std=torch.tensor((1,1,1)).float().view(3,1,1).cuda()
else:
    mu = torch.tensor(cifar10_mean).view(3,1,1).cuda()
    std = torch.tensor(cifar10_std).view(3,1,1).cuda()
...
def get_fl(output,y,n_bin,criterion,bl):
    n_bin=9
    bin_sum = [0]*n_bin
    cnt = 0
    for i in range(0, n_bin):
        ...
        y_mask = y[mask]               
        if not mask.float().sum() == 0:
            bin_sum[i] += criterion(output_mask, y_mask)
            cnt+=1
    fl_loss=torch.abs(sum(bin_sum)/cnt - criterion(output, y))
    return fl_loss
...
def main():
    ...
    if args.dataset=='UTK':
        dataset = pd.read_csv('age_gender.gz', compression='gzip')
        age_bins = [0,10,15,20,25,30,40,50,60,120]
        age_labels = [0, 1, 2, 3, 4, 5, 6, 7, 8]
        dataset['agebin'] = pd.cut(dataset.age, bins=age_bins, labels=age_labels)
        dataset = dataset.drop(['age', 'gender', 'img_name'], axis=1)

        train_set, test_set = train_test_split(dataset, test_size=0.2)
        train_transform = torchvision.transforms.Compose([torchvision.transforms.ToTensor()])
        test_transform = torchvision.transforms.Compose([torchvision.transforms.ToTensor()])
        trainset = UTKDataset(train_set, transform=train_transform)
        testset = UTKDataset(test_set, transform=test_transform)
        train_batches = DataLoader(trainset, batch_size=32, shuffle=True)
        test_batches = DataLoader(testset, batch_size=32, shuffle=False)
    else:
    ...
    logger.info('Epoch \t Train Time \t Test Time \t LR \t \t Train Loss \t Train Acc \t Train Robust Loss \t Train Robust Acc \t Train_FairniessLoss \t Test Loss \t Test Acc \t Test Robust Loss \t Test Robust Acc \t Test_FairniessLoss')
    for epoch in range(start_epoch, epochs):
        ...
        for i, batch in enumerate(train_batches):
            ...
            if args.dataset=='UTK':
                fl_loss=get_fl(output,y,n_bin=9,criterion=criterion,bl=bl)
            else:
                raise NotImplementedError("Not implemented!")
            scale = {}
            if args.method=='MGDA':
                pareto=[]
                if args.n_tasks==2:
                    losses_all=[robust_loss,fl_loss]
                    loss_data=[robust_loss.item(),fl_loss.item()]
                else:
                    raise NotImplementedError("not implemented!")
                for i_loss in losses_all:
                    pareto.append(torch.autograd.grad(i_loss, model.parameters())[0])
                gn=gradient_normalizers(pareto,loss_data,args.gradnorm)
                for t in args.n_tasks:
                    for gr_i in range(len(pareto[t])):
                        pareto[t][gr_i] = pareto[t][gr_i] / gn[t]
                
                sol, min_norm = MinNormSolver.find_min_norm_element([pareto[t] for t in args.n_tasks])
                for i, t in enumerate(args.n_tasks):
                    scale[t] = float(sol[i])
            else:
                scale[0]=1.0
            ...
            if args.n_tasks==2:
               loss=fl_loss*scale[0]+robust_loss*scale[1]
            else:
                raise NotImplementedError("Not Implemented!")
        ...
        if not args.eval:
            logger.info('%d \t %.1f \t \t %.1f \t \t %.4f \t %.4f \t %.4f \t %.4f \t \t %.4f \t \t %.4f \t %.4f \t %.4f \t %.4f \t %.4f \t \t %.4f',
                ...
                train_loss/train_n, train_acc/train_n, train_robust_loss/train_n,train_fl_loss/train_batch_n, train_robust_acc/train_n,
                test_loss/test_n, test_acc/test_n, test_robust_loss/test_n, test_robust_acc/test_n,test_fl_loss/test_batch_n)
